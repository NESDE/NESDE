...
-- DROP TABLE IF EXISTS hep_minilabevents;
-- CREATE TABLE hep_minilabevents AS (SELECT le.*,hs.stay_id FROM hep_miniicustays hs LEFT JOIN mimic_hosp.labevents le ON le.hadm_id = hs.hadm_id);
...
(
	SELECT tmw.stay_id
		...
		, ml.valueuom
	FROM timewinds tmw
	LEFT JOIN mimic_hosp.labevents ml
