...
def plot_data_NESDE(model, trj, dt, t_end=None):
    with torch.no_grad():
        ...
        for i,time in enumerate(ts_to_plot):
            ...
            # nll_loss = (1.0 / dim_obs) * model.loss_fn(pred_mu, smp, pred_var)
...
def eval_NESDE(model, dl, Tmin=None, n_samp_per_traj=None, verbose=0, detailed_res=False, skip_first=False):
    with torch.no_grad():
        t0 = time.time()
        model.eval()
            ...
            for trj in batch:
                ...
                for id in (traj_id):
                    ...
                    if model.dt is None or model.B is None:
                        ...
                        # St, St_var = model(St, St_var, ttimes - prev_time, U=[lambda t: trj['U'](prev_time + t)])
                    prev_time = ttimes
                ...
                if n_obs > 0:
                    ...
                    CORRs.append(np.corrcoef((preds_mu, [o[0] for o in trj['obs']]))[0,1] if n_obs>1 else None)
...
def train_NESDE(model, dl, optim, train_epochs, w_path, random_samples=False, eval_rate=1, patience=200,
                sched=None, train_nll=True, valid_by_nll=True, Tmin=None, n_samp_per_traj=None, verbose=1, log_rate=10):
    ...
    for t in range(train_epochs):
        ...
        if (t + 1) % eval_rate == 0:
            dl.valid()
            valid_nll, valid_mse, _ = eval_NESDE(model, dl, Tmin=Tmin, n_samp_per_traj=n_samp_per_traj)
...
def eval_contextual_NESDE(model,dl,deterministic=False,save_data=False):
    with torch.no_grad():
        model.eval()
        NLLs, MSEs, CORRs = [], [], []
        n_batches = 0
        ...
        while not epoch_done:
            ...
            for trj in batch:
                ...
                for id in (traj_id):
                    ...
                    nll_loss = (1.0 / len(traj_id)) * nll_loss
                    mse_loss = (1.0 / len(traj_id)) * mse_loss
                    trj_nll += nll_loss.item()
                    trj_mse += mse_loss.item()
                NLLs.append(trj_nll)
                MSEs.append(trj_mse)
                CORRs.append(np.corrcoef((preds_mu, [o[0] for o in trj['obs']]))[0,1])
                if save_data:
...
def train_contextual_NESDE(model, dl, optim, train_epochs, w_path, random_samples=False, eval_rate=1, patience=200,
                sched=None, verbose=1,deterministic=False):
    iterations, epochs, groups, NLLs, MSEs = [], [], [], [], []
    iter = -1
    pat_count = 0
    best_valid_loss = np.inf
    ...
    for t in range(train_epochs):
            ...
            for trj in batch:
                ...
                for id in (traj_id):
                    ...
                    if deterministic:
                        ...
                        pred_var = torch.abs(torch.diagonal(St_var, dim1=-2, dim2=-1)[..., smp_mask].view(-1, dim_obs))
                        loss = model.loss_fn(pred_mu, smp, pred_var)
                        mseloss = mse_fn(pred_mu.view(-1),smp.view(-1))#(1.0 / dim_obs) * torch.sum(torch.square(pred_mu - smp))
                    if torch.isnan(loss):
                        loss = torch.zeros(1,device=model.device,requires_grad=True)
                    if not deterministic:
                        St, St_var = model.conditional_dist(St, St_var, smp_mask, smp)
        ...
        if (t + 1) % eval_rate == 0:
            dl.valid()
            valid_nll, valid_mse, _ = eval_contextual_NESDE(model, dl, deterministic=deterministic)
            valid_nll, valid_mse = np.mean(valid_nll), np.mean(valid_mse)
